...
# I create another class FewVLMTokenizer extending it to add <vis_extra_id_{}>

class FewVLMTokenizer(T5Tokenizer):
...
class FewVLMConverter(SpmConverter):
...
def convert_slow_fewvlmtokenizer(fewvlmtokenizer):
    return FewVLMConverter(fewvlmtokenizer).converted()


class FewVLMTokenizerFast(T5TokenizerFast):

    # vocab_files_names = VOCAB_FILES_NAMES
    # pretrained_vocab_files_map = PRETRAINED_VOCAB_FILES_MAP
    # max_model_input_sizes = PRETRAINED_POSITIONAL_EMBEDDINGS_SIZES
    # model_input_names = ["attention_mask"]
    slow_tokenizer_class = FewVLMTokenizer
